...
async function fetchProducts() {
  try {
      const response = await fetch(URL_PRODUCTS); // Reemplaza con la URL de tu API
      const data = await response.json();

      // Convertimos cada objeto de datos en una instancia de Product
      const products = data.map(productData => new Product(
          ...
          productData.price,
          productData.img,
          productData.unitsPackage,
          productData.dateCreation
      ));
...
function displayProducts(products) {
  ...
  container.innerHTML = ""; // Limpiamos el contenedor antes de mostrar

  // Iteramos sobre cada instancia de Product y llamamos a displayInfo()
  products.forEach(product => {
      container.innerHTML += product.displayInfo();
  });
}

// Ejecuta fetchProducts() al cargar la página
